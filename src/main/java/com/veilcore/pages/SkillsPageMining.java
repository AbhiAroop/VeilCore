...
public class SkillsPageMining extends InteractiveCustomUIPage<SkillsPageMining.SkillsEventData> {
    ...
    public SkillsPageMining(@Nonnull PlayerRef playerRef, Profile profile) {
    ...
    public void build(
        ...
        cmd.append("Pages/SkillsPageMining.ui");
        ...
        // Set level info
        cmd.set("#CurrentLevel.Text", String.valueOf(miningLevel.getLevel()));
        if (miningLevel.isMaxLevel()) {
            cmd.set("#NextLevel.Text", "MAX");
            cmd.set("#XPToNext.Text", "0");
            cmd.set("#Progress.Text", "100%");
        } else {
            cmd.set("#NextLevel.Text", String.valueOf(miningLevel.getLevel() + 1));
            cmd.set("#XPToNext.Text", String.valueOf(miningLevel.getXpToNextLevel()));
            cmd.set("#Progress.Text", String.format("%.1f%%", miningLevel.getProgressPercent() * 100));
        }
        
        cmd.set("#CurrentXP.Text", String.valueOf(miningLevel.getCurrentXp()));
        
        // Calculate total XP
        long totalXp = SkillLevel.getTotalXpForLevel(miningLevel.getLevel()) + miningLevel.getCurrentXp();
        cmd.set("#TotalXP.Text", String.valueOf(totalXp));
        
        // Set token counts
        Map<TokenTier, Integer> tokens = skills.getTreeData().getAllTokenCounts(Skill.MINING.getId());
