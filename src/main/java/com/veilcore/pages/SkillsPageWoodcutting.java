...
public class SkillsPageWoodcutting extends InteractiveCustomUIPage<SkillsPageWoodcutting.SkillsEventData> {
    ...
    public SkillsPageWoodcutting(@Nonnull PlayerRef playerRef, Profile profile) {
    ...
    public void build(
        ...
        cmd.append("Pages/SkillsPageWoodcutting.ui");
        
        ProfileSkills skills = profile.getSkills();
        SkillLevel woodcuttingLevel = skills.getSkillLevel(Skill.WOODCUTTING);
        ...
        // Set level info
        cmd.set("#CurrentLevel.Text", String.valueOf(woodcuttingLevel.getLevel()));
        if (woodcuttingLevel.isMaxLevel()) {
            cmd.set("#NextLevel.Text", "MAX");
            cmd.set("#XPToNext.Text", "0");
            cmd.set("#Progress.Text", "100%");
        } else {
            cmd.set("#NextLevel.Text", String.valueOf(woodcuttingLevel.getLevel() + 1));
            cmd.set("#XPToNext.Text", String.valueOf(woodcuttingLevel.getXpToNextLevel()));
            cmd.set("#Progress.Text", String.format("%.1f%%", woodcuttingLevel.getProgressPercent() * 100));
        }
        
        cmd.set("#CurrentXP.Text", String.valueOf(woodcuttingLevel.getCurrentXp()));
        
        // Calculate total XP
        long totalXp = SkillLevel.getTotalXpForLevel(woodcuttingLevel.getLevel()) + woodcuttingLevel.getCurrentXp();
        cmd.set("#TotalXP.Text", String.valueOf(totalXp));
        
        // Set token counts
        Map<TokenTier, Integer> tokens = skills.getTreeData().getAllTokenCounts(Skill.WOODCUTTING.getId());
